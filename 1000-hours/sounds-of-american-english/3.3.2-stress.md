...
英语里除了重音、次重音、非重音之外，还有**弱读音节**，比如被弱化成 `ə` 的 *schwa*。
...
细细体会 *individual* `/ˌɪndəˈvɪdʒəwəl/` 这个词，如果每个音节都清晰地用同样的 stress 读出来的话，应该是 `/ɪn-dɪ-vɪ-dʒju-əl/`…… 这里面，第二个音节 `/dɪ/`里的元音 `ɪ` 被弱化为 `ə`；另外，第四个音节 `/dʒju/` 里的元音 `ju` 也被弱化成了 `ə`，但保留了 `u` 的音色；而处于音节末尾的 `l`，之前自带着轻微的 `ə`，于是连成了很轻的 `wəl`……
